...
export function CommunicationHub() {
  ...
  return (
    <div className="flex h-full border border-zinc-900 rounded-3xl overflow-hidden bg-zinc-950/50 backdrop-blur-sm">
      {/* Context Sidebar */}
      <div className="w-80 border-r border-zinc-900 flex flex-col bg-zinc-950">
        <div className="p-6 border-b border-zinc-900">
          <h2 className="text-sm font-semibold text-zinc-200">Contexts</h2>
        </div>
        <div className="flex-1 overflow-y-auto">
          {contexts.map((context) => (
            <button
              ...
              className={cn(
                "w-full p-4 flex flex-col gap-1 border-b border-zinc-900/50 hover:bg-zinc-900/50 transition-colors text-left",
                activeContext.id === context.id && "bg-zinc-900/80"
              )}
            >
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] text-zinc-600">{context.timestamp}</span>
              </div>
              <h4 className="text-sm font-medium text-zinc-300 line-clamp-1">{context.title}</h4>
      ...
      <div className="flex-1 flex flex-col bg-zinc-900/10">
        <header className="p-6 border-b border-zinc-900 flex justify-between items-center bg-zinc-950/50">
          <div>
            <h3 className="text-lg font-medium text-zinc-200">{activeContext.title}</h3>
            <p className="text-xs text-zinc-500">Linked to {activeContext.type} #{activeContext.id}</p>
          </div>
          <div className="flex gap-2">
            <button className="p-2 rounded-lg bg-zinc-800 text-zinc-400 hover:text-zinc-200 transition-colors">
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-8">
          {activeContext.type === "meeting" && (
            <div className="space-y-6">
              <div className="p-6 rounded-2xl bg-zinc-800/20 border border-zinc-800/50">
                <div className="flex items-center gap-2 mb-4">
                  <FileText className="w-4 h-4 text-purple-400" />
                  <h4 className="text-xs uppercase tracking-[0.2em] font-bold text-purple-400">AI Meeting Extract</h4>
                </div>
          ...
          <div className="flex gap-4 max-w-2xl">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="p-4 rounded-2xl rounded-tl-none bg-zinc-800/50 border border-zinc-800 text-zinc-300 text-sm">
        ...
        <div className="p-6 pt-0 mt-auto">
          <div className="relative">
            <input 
              ...
              className="w-full bg-zinc-950 border border-zinc-800 rounded-full px-6 py-3 text-sm text-zinc-300 focus:outline-none focus:border-zinc-700 transition-colors"
            />
            <button className="absolute right-2 top-1.5 p-1.5 rounded-full bg-zinc-100 text-zinc-950">
